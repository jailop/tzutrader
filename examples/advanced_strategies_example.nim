...
proc demonstrateTrendStrengthStrategy() =
  ...
  echo pad("Bar", 6) & pad("Price", 10, true) & pad("KAMA", 10, true) & pad(
      "StochRSI", 10, true) & pad("ADX", 10, true) & pad("Signal", 14, true)
  echo "-".repeat(70)
  ...
  for i in 0..<prices.len:
    ...
    if signal != "HOLD" or i mod 10 == 0:
      let kamaStr = if kamaVal.isNaN: "warmup" else: formatFloat(kamaVal,
          ffDecimal, 2)
      let stochStr = if stochRsiVal.k.isNaN: "warmup" else: formatFloat(
          stochRsiVal.k, ffDecimal, 1)
      let adxStr = if adxVal.adx.isNaN: "warmup" else: formatFloat(adxVal.adx,
          ffDecimal, 1)
      let priceStr = formatFloat(prices[i], ffDecimal, 2)
      echo pad($i, 6) & pad(priceStr, 10, true) & pad(kamaStr, 10, true) & pad(
          stochStr, 10, true) & pad(adxStr, 10, true) & pad(signal, 14, true)
  ...
  for i, trade in trades:
    ...
    echo "Trade " & $(i+1) & ": Entry bar " & $trade.entry & ", Exit bar " &
        $trade.exit & ", P&L: " & profitSign & profitStr & "%"
  ...
  if trades.len > 0:
    ...
    echo "\nTotal P&L: " & totalProfitSign & formatFloat(totalProfit, ffDecimal,
        2) & "%"
    echo "Average P&L per trade: " & avgProfitSign & formatFloat(avgProfit,
        ffDecimal, 2) & "%"
    echo "Win Rate: " & formatFloat(winRate, ffDecimal, 1) & "% (" &
        $winningTrades & "/" & $trades.len & ")"
...
proc demonstrateMomentumRotation() =
  ...
  for symbol in symbols:
    ppoIndicators[symbol] = newPPO(fastPeriod = 12, slowPeriod = 26,
        signalPeriod = 9)
  ...
  for i in 0..<60:
    ...
    let energyBase = 100.0 + (if i < 20: float64(i) * 0.1 else: float64(i -
        20) * 1.2 + 2.0)
    ...
    let financeBase = 100.0 + (if i < 40: float64(i) * 0.1 else: float64(i -
        40) * 1.3 + 4.0)
  ...
  echo pad("Bar", 6) & pad("Asset", 10, true) & pad("Price", 10, true) & pad(
      "PPO", 10, true) & pad("AROON", 10, true) & pad("Action", 14, true)
  echo "-".repeat(70)
  ...
  for i in 0..<60:
    ...
    for symbol in symbols:
      ...
      let aroonVal = aroonIndicators[symbol].update(highs[symbol][i], lows[
          symbol][i])
    ...
    if i > 25 and bestSymbol != "" and bestSymbol != currentHolding: # Allow warmup
      if currentHolding == "":
        ...
        rotations.add((bar: i, fromSymbol: currentHolding,
            toSymbol: bestSymbol))
    ...
    if rotations.len > 0 and rotations[^1].bar == i:
      ...
      echo "\n" & pad($i, 6) & " ROTATION: " & rotation.fromSymbol & " -> " &
          rotation.toSymbol
      echo "-".repeat(70)
    ...
    if i > 25 and (i mod 5 == 0 or (rotations.len > 0 and rotations[^1].bar == i)):
      for symbol in symbols:
        let ppoStr = if symbol in ppoValues and not ppoValues[symbol].isNaN:
                      formatFloat(ppoValues[symbol], ffDecimal,
                          2) else: "warmup"
        let aroonVal = aroonIndicators[symbol][0]
        let aroonStr = if not aroonVal.up.isNaN: formatFloat(aroonVal.up,
            ffDecimal, 0) else: "warmup"
        let action = if symbol == currentHolding: ">>> HOLD" else: ""
        let priceStr = formatFloat(prices[symbol][i], ffDecimal, 2)
        echo pad($i, 6) & pad(symbol, 10, true) & pad(priceStr, 10, true) & pad(
            ppoStr, 10, true) & pad(aroonStr, 10, true) & pad(action, 14, true)

  printSubSection("Rotation Summary")
  echo "Total Rotations: " & $rotations.len
  for i, rotation in rotations:
    echo "Rotation " & $(i+1) & " at bar " & $rotation.bar & ": " &
        rotation.fromSymbol & " -> " & rotation.toSymbol
...
proc demonstrateVolatilityAdjusted() =
  ...
  var trades: seq[tuple[bar: int, price: float64, natr: float64, shares: int,
      riskAmount: float64]] = @[]
  ...
  echo "Target Risk per Trade: " & formatFloat(targetRiskPercent, ffDecimal,
      0) & "% ($" & formatFloat(targetRiskDollars, ffDecimal, 0) & ")"
  echo ""
  echo pad("Bar", 6) & pad("Price", 10, true) & pad("NATR %", 10, true) & pad(
      "Shares", 10, true) & pad("Position", 12, true) & pad("Risk $", 12, true)
  echo "-".repeat(70)
    ...
    if not rsiVal.isNaN and i > 14:
      if rsiVal < 30 or rsiVal > 70: # Trade signal
        if not natrVal.isNaN and natrVal > 0:
          ...
          trades.add((bar: i, price: prices[i], natr: natrVal, shares: shares,
          ...
          echo pad($i, 6) & pad(priceStr, 10, true) & pad(natrStr, 10, true) &
              pad($shares, 10, true) & pad("$" & posStr, 12, true) & pad("$" &
              riskStr, 12, true)
  ...
  if lowVolTrades.len > 0:
    let avgShares = lowVolTrades.mapIt(float64(it.shares)).foldl(a + b, 0.0) /
        float64(lowVolTrades.len)
    let avgNatr = lowVolTrades.mapIt(it.natr).foldl(a + b, 0.0) / float64(
        lowVolTrades.len)
  ...
  if highVolTrades.len > 0:
    let avgShares = highVolTrades.mapIt(float64(it.shares)).foldl(a + b, 0.0) /
        float64(highVolTrades.len)
    let avgNatr = highVolTrades.mapIt(it.natr).foldl(a + b, 0.0) / float64(
        highVolTrades.len)
...
proc demonstrateDivergenceDetection() =
  ...
  echo pad("Bar", 6) & pad("Price", 10, true) & pad("MOM", 10, true) & pad(
      "CMO", 10, true) & pad("Signal", 22, true)
  echo "-".repeat(70)
  ...
  for i in 0..<prices.len:
    ...
    if i >= 20:
      ...
      if i >= 20 + lookbackBars:
        ...
        let prevMomHigh = max(momValues[
            i-recentBars-lookbackBars+1..i-recentBars])
    ...
    if signal != "" or i mod 10 == 0:
      let momStr = if momVal.isNaN: "warmup" else: formatFloat(momVal,
          ffDecimal, 2)
      let cmoStr = if cmoVal.isNaN: "warmup" else: formatFloat(cmoVal,
          ffDecimal, 1)
      let priceStr = formatFloat(prices[i], ffDecimal, 2)
      echo pad($i, 6) & pad(priceStr, 10, true) & pad(momStr, 10, true) & pad(
          cmoStr, 10, true) & pad(signal, 22, true)
  ...
  for i, sig in signals:
    ...
    echo "Signal " & $(i+1) & " at bar " & $sig.bar & ": " & sig.signalType &
        " divergence at price " & priceStr
  ...
  if signals.len > 0:
    ...
    for sig in signals:
      ...
      echo "  " & sig.signalType & " at bar " & $sig.bar & ": Price moved " &
          movementSign & movementStr & " (" & actualDirection & ") - " & correctStr
