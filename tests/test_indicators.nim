...
suite "TRIMA (Triangular Moving Average)":
  ...
  test "TRIMA is smoother than SMA":
    ...
    let data = @[100.0, 110.0, 105.0, 115.0, 108.0, 118.0, 112.0, 120.0, 115.0, 125.0, 120.0]
...
suite "ADX (Average Directional Movement Index)":
  test "Basic calculation with warmup":
    ...
    for i in 0..<period*2:
      let result = adx.update(105.0 + float64(i), 95.0 + float64(i), 100.0 + float64(i))
      if i < period:
...
# ============================================================================
# Phase 9.4 Tests: Additional Momentum Indicators
# ============================================================================
...
suite "STOCHRSI (Stochastic RSI)":
  ...
  test "StochRSI responds to RSI changes":
    ...
    for i in 0..<60:
      let price = if i mod 4 < 2: 100.0 + float64(i div 4) else: 100.0 + float64(i div 4) + 5.0
  ...
  test "StochRSI memory access":
    var stochRsi = newSTOCHRSI(rsiPeriod = 14, period = 14, kPeriod = 3, dPeriod = 3, memSize = 10)
