...
suite "StochasticStrategy Tests":
  test "Basic construction and parameters":
    let strat = newStochasticStrategy(kPeriod=14, dPeriod=3, oversold=20.0, overbought=80.0)
    check strat.name() == "Stochastic Oscillator Strategy"
  
  test "Construction with custom parameters":
    let strat = newStochasticStrategy(kPeriod=21, dPeriod=5, oversold=30.0, overbought=70.0)
    check strat.name() == "Stochastic Oscillator Strategy"
...
suite "AroonStrategy Tests":
  test "Basic construction and parameters":
    let strat = newAroonStrategy(period=25, upThreshold=70.0, downThreshold=30.0)
    check strat.name() == "Aroon Strategy"
  
  test "Construction with custom parameters":
    let strat = newAroonStrategy(period=14, upThreshold=80.0, downThreshold=20.0)
    check strat.name() == "Aroon Strategy"
