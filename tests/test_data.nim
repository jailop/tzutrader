...
suite "Cache Management Tests":
  ...
  test "Add to cache and check":
    ...
    let data = @[
      OHLCV(timestamp: now - 86400, open: 100.0, high: 105.0, low: 99.0,
          close: 103.0, volume: 1000000.0),
      OHLCV(timestamp: now, open: 103.0, high: 108.0, low: 102.0, close: 106.0,
          volume: 1100000.0)
    ]
  ...
  test "Get cached data in range":
    ...
    let data = @[
      OHLCV(timestamp: now - 2 * 86400, open: 98.0, high: 102.0, low: 97.0,
          close: 100.0, volume: 900000.0),
      OHLCV(timestamp: now - 86400, open: 100.0, high: 105.0, low: 99.0,
          close: 103.0, volume: 1000000.0),
      OHLCV(timestamp: now, open: 103.0, high: 108.0, low: 102.0, close: 106.0,
          volume: 1100000.0)
  ...
  test "Cache with useCache=false":
    ...
    let data = @[
      OHLCV(timestamp: now, open: 100.0, high: 105.0, low: 99.0, close: 103.0,
          volume: 1000000.0)
    ]
...
suite "CSV File I/O Tests":
  ...
  test "Write and read CSV file":
    let testData = @[
      OHLCV(timestamp: 1000, open: 100.0, high: 105.0, low: 95.0, close: 102.0,
          volume: 1000000.0),
      OHLCV(timestamp: 2000, open: 102.0, high: 107.0, low: 100.0, close: 104.0,
          volume: 1500000.0),
      OHLCV(timestamp: 3000, open: 104.0, high: 109.0, low: 102.0, close: 106.0,
          volume: 2000000.0)
    ]
