...
proc createTestData(bars: int, startPrice: float64 = 100.0,
    trend: float64 = 0.0): seq[OHLCV] =
...
suite "Backtester Construction Tests":
  ...
  test "Create backtester with custom parameters":
    let strategy = newRSIStrategy()
    let bt = newBacktester(strategy, initialCash = 50000.0, commission = 0.001,
        verbose = true)
...
suite "Backtest Execution Tests":
  ...
  test "Run backtest on uptrend data":
    let strategy = newRSIStrategy(period = 14, oversold = 30.0,
        overbought = 70.0)
  ...
  test "Backtest with no signals generates no trades":
    ...
    let strategy = newRSIStrategy(period = 14, oversold = 10.0,
        overbought = 90.0)
...
suite "Report Generation Tests":
  ...
  test "Report calculates returns correctly":
    ...
    let expectedReturn = ((report.finalValue - report.initialCash) /
        report.initialCash) * 100.0
...
suite "Strategy Integration Tests":
  ...
  test "RSI strategy backtest":
    ...
    let report = quickBacktest("RSI_TEST", strategy, data,
        initialCash = 10000.0)
  ...
  test "MACD strategy backtest":
    ...
    let report = quickBacktest("MACD_TEST", strategy, data,
        initialCash = 10000.0)
...
suite "Risk Management Integration Tests":
  ...
  test "Fixed stop-loss exits are tracked":
    ...
    if report.totalTrades > 0:
      check (report.stopLossExits + report.takeProfitExits +
          report.strategyExits) >= 0
