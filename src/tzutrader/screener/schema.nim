...
proc newBuiltInStrategy*(name: string,
                        params: Table[string, ParamValue] = initTable[string,
                            ParamValue]()): ScreenerStrategyConfig =
...
proc newCsvDataConfig*(directory: string,
                      lookback: Option[LookbackPeriod] = none(
                          LookbackPeriod)): ScreenerDataConfig =
...
proc parseLookbackPeriod*(s: string): LookbackPeriod =
  ...
  if numEnd == 0:
    raise newException(ScreenerConfigError,
        &"Invalid lookback period: {s} (no numeric value)")
  ...
  let value = try:
    ...
    raise newException(ScreenerConfigError,
        &"Invalid numeric value in lookback period: {valueStr}")
  ...
  let unit = case unitStr
    ...
    of "y": tuYears
      ...
      raise newException(ScreenerConfigError,
          &"Invalid time unit: {unitStr} (use m/h/d/w/mo/y)")
...
proc parseTimeInterval*(s: string): Interval =
  ...
  of "1mo": Int1mo
    ...
    raise newException(ScreenerConfigError,
        &"Invalid time interval: {s} (use 1m/5m/15m/30m/1h/1d/1wk/1mo)")
