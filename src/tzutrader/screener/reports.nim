...
proc formatTerminalTable*(alerts: seq[Alert],
    detailLevel: DetailLevel = dlSummary): string =
  ## Formats alerts as a terminal table with optional color support
  if alerts.len == 0:
    return colorYellow() & "No alerts to display" & colorReset() & "\n"

  # Header
  let totalWidth = ColSymbol + ColStrategy + ColSignal + ColStrength +
      ColPrice + ColIndicators + 12
  ...
  for alert in alerts:
    ...
    result.add(
      ...
      " " & typeColor & ($alert.alertType).alignLeft(ColSignal - 1) &
          colorReset() & VerticalLine &
      " " & strengthColor & ($alert.strength).alignLeft(ColStrength - 1) &
          colorReset() & VerticalLine &
...
proc generateReport*(alerts: seq[Alert], summary: ScreenerSummary,
...
export printSummary, formatTerminalTable, formatCsvReport, formatJsonReport,
    formatMarkdownReport, printDetailedAlerts
