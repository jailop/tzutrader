...
import ../data
import ../strategy
import ../declarative/[parser, validator, strategy_builder,
    schema as declSchema]
...
proc loadStrategy(config: ScreenerStrategyConfig): tuple[name: string,
    strategy: Strategy] =
  ...
  of skBuiltIn:
    ...
    let strategy = case config.name.toLowerAscii()
      of "rsi":
        ...
        let oversold = if params.hasKey("oversold"): params[
            "oversold"] else: 30.0
        let overbought = if params.hasKey("overbought"): params[
            "overbought"] else: 70.0
      ...
      of "crossover":
        let fastPeriod = if params.hasKey("fastPeriod"): int(params[
            "fastPeriod"]) else: 50
        let slowPeriod = if params.hasKey("slowPeriod"): int(params[
            "slowPeriod"]) else: 200
        newCrossoverStrategy(fastPeriod, slowPeriod)

      of "stochastic":
        let kPeriod = if params.hasKey("kPeriod"): int(params[
            "kPeriod"]) else: 14
        let dPeriod = if params.hasKey("dPeriod"): int(params[
            "dPeriod"]) else: 3
        let oversold = if params.hasKey("oversold"): params[
            "oversold"] else: 20.0
        let overbought = if params.hasKey("overbought"): params[
            "overbought"] else: 80.0
      ...
      of "cci":
        ...
        let oversold = if params.hasKey("oversold"): params[
            "oversold"] else: -100.0
        let overbought = if params.hasKey("overbought"): params[
            "overbought"] else: 100.0
        newCCIStrategy(period, oversold, overbought)

      else:
        raise newException(ScreenerError,
            &"Unknown built-in strategy: {config.name}")
...
proc assessStrength*(signal: Signal, indicators: Table[string,
    float64]): AlertStrength =
...
proc run*(screener: var Screener): ScreenerResult =
  ...
  # Generate summary
  let summary = generateSummary(filteredAlerts, dataMap.len,
      screener.strategies.len)
...
proc validateConfig*(config: ScreenerConfig): tuple[valid: bool, errors: seq[string]] =
  ...
  case config.data.source
  of dsYahoo:
    let barCount = lookbackToBarCount(config.data.lookback,
  ...
  of dsCoinbase:
    let barCount = lookbackToBarCount(config.data.lookbackCB,
        config.data.intervalCB)
    if barCount < 20:
      result.errors.add(&"Warning: Only {barCount} bars available - may be insufficient for some indicators")
...
proc formatResult*(screenerResult: ScreenerResult,
    config: ScreenerOutputConfig): string =
...
proc printResult*(screenerResult: ScreenerResult,
    config: ScreenerOutputConfig) =
