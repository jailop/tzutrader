## Stochastic Oscillator Strategy for tzutrader
##
## Mean reversion/momentum strategy based on Stochastic Oscillator.
##
## **Strategy Type**: Mean Reversion / Momentum
##
## **Best Market Conditions**: Ranging markets with clear overbought/oversold levels
##
## **Trading Logic**:
## - Buy when %K crosses above %D in oversold zone (<20)
## - Sell when %K crosses below %D in overbought zone (>80)
## - Crossover must occur in extreme zones for signal validity
##
## **Typical Parameters**:
## - kPeriod: 14 (lookback period for %K)
## - dPeriod: 3 (smoothing period for %D)
## - oversold: 20.0 (lower threshold)
## - overbought: 80.0 (upper threshold)
##
## **Risk Profile**: Moderate, good for range-bound markets
##
## **Complementary Strategies**: Works well with trend filters or volume confirmation
##
## **Known Limitations**:
## - Can give false signals in strong trends
## - Overbought/oversold conditions can persist in trends
## - Consider waiting for crossover in extreme zones rather than just zone entry
...
proc onData*(s: StochasticStrategy, bar: OHLCV): Signal =
  ...
  if not stochResult.k.isNaN and not stochResult.d.isNaN:
    ...
    if not s.lastKAboveD and currentKAboveD and (inOversold or s.lastInOversold):
    ...
    elif s.lastKAboveD and not currentKAboveD and (inOverbought or s.lastInOverbought):
