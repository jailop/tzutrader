...
proc checkRiskRules(bt: Backtester, bar: OHLCV, symbol: string) =
  ...
  if tracker.takeProfit != nil:
    let (triggered, exitPercent) = tracker.takeProfit.checkTakeProfit(tracker.state, indicators)
    if triggered:
      ...
      if tracker.takeProfit.kind == tpkMultiLevel and exitPercent < 100.0:
        ...
        let profitPct = (tracker.state.currentPrice - tracker.state.entryPrice) / tracker.state.entryPrice * 100.0
...
proc run*(bt: Backtester, data: seq[OHLCV], symbol: string = ""): BacktestReport =
  ...
  let avgStopLossReturn = if stopLossExits > 0 and tradePnLs.len > 0:
    ...
    if losingTrades.len > 0: losingTrades.sum() / losingTrades.len.float64 else: 0.0
  else:
    0.0
  ...
  let avgTakeProfitReturn = if takeProfitExits > 0 and tradePnLs.len > 0:
    ...
    if winningTrades.len > 0: winningTrades.sum() / winningTrades.len.float64 else: 0.0
  else:
    0.0
