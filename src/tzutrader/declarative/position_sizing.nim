...
##
## This module implements various position sizing strategies for controlling
## how much capital to allocate to each trade.
##
## Supported strategies:
## - Fixed percentage of capital
## - Fixed number of shares
## - Risk-based sizing (based on volatility/ATR)
## - Equal weight (for multi-asset portfolios)
...
proc newRiskBasedSizer*(riskPerTrade: float, atrMultiplier: float, atrIndicatorId: string = "atr_14"): RiskBasedSizer =
...
proc newKellySizer*(winRate: float, avgWinLoss: float, kellyFraction: float = 0.25): KellySizer =
...
method calculateShares*(
  ...
  if not indicators.hasKey(sizer.atrIndicatorId):
    raise newException(SizingError, "ATR indicator not found: " & sizer.atrIndicatorId)
...
method calculateShares*(
  ...
  let kellyPct = (sizer.winRate * sizer.avgWinLoss - (1.0 - sizer.winRate)) / sizer.avgWinLoss
...
proc calculateMaxShares*(capital: float, price: float, maxPercentage: float = 100.0): int =
