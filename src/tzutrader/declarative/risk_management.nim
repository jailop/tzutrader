...
##
## This module implements various risk management rules for protecting capital
## and locking in profits.
##
## Stop Loss Types:
## - Fixed percentage (e.g., exit if loss > 5%)
## - Fixed price (absolute price level)
## - Trailing stop (trails with profit)
...
proc newTrailingStopLoss*(trailPercentage: float, activationProfit: float = 0.0): TrailingStopLoss =
...
proc newATRBasedStopLoss*(atrMultiplier: float, atrIndicatorId: string = "atr_14"): ATRBasedStopLoss =
...
proc newRiskRewardTakeProfit*(ratio: float, stopLossRule: StopLossRule): RiskRewardTakeProfit =
...
proc newMultiLevelTakeProfit*(levels: seq[TakeProfitLevel]): MultiLevelTakeProfit =
...
proc calculateStopPrice*(rule: StopLossRule, entryPrice: float, indicators: Table[string, float] = initTable[string, float]()): Option[float] =
...
proc calculateTakeProfitPrice*(rule: TakeProfitRule, entryPrice: float, indicators: Table[string, float] = initTable[string, float]()): Option[float] =
