...
proc addWarning*(vr: var ValidationResult, level: WarningLevel, msg: string,
    suggestion: string = "") =
  ## Add a warning to validation result (Phase 2 - Feature B3)
  vr.warnings.add(ValidationWarning(level: level, message: msg,
...
proc validateCondition*(condition: ConditionYAML, indicatorIds: HashSet[
  ...
  of ckSimple:
    ...
    if not isValidRef(condition.left):
      result.addError(formatError("Undefined reference in condition: " &
          condition.left, condition.location))
    if not isValidRef(condition.right):
      result.addError(formatError("Undefined reference in condition: " &
          condition.right, condition.location))
  of ckAnd:
    if condition.andConditions.len == 0:
      result.addError(formatError("AND condition must have at least one child condition",
          condition.location))
  ...
  of ckOr:
    if condition.orConditions.len == 0:
      result.addError(formatError("OR condition must have at least one child condition",
          condition.location))
    for child in condition.orConditions:
      ...
      if not childResult.valid:
        for err in childResult.errors:
          result.addError(err)
  of ckNot:
    result.addError(formatError("NOT conditions not supported in Phase 1",
        condition.location))

proc validateRule*(rule: RuleYAML, indicatorIds: HashSet[
    string]): ValidationResult =
...
proc validateStrategy*(strategy: StrategyYAML): ValidationResult =
  ...
  of psPercent:
    if strategy.positionSizing.percentCapital <= 0 or
        strategy.positionSizing.percentCapital > 100:
...
proc generateWarnings*(vr: var ValidationResult, strategy: StrategyYAML) =
  ...
  of psPercent:
    if strategy.positionSizing.percentCapital > 50:
      vr.addWarning(
        wlHigh,
        "Position size is very high (" &
        $strategy.positionSizing.percentCapital & "%). " &
      ...
      vr.addWarning(
        wlMedium,
        "Position size is above recommended range (" &
        $strategy.positionSizing.percentCapital & "%).",
  ...
  for ind in strategy.indicators:
    if ind.params.hasKey("period"):
      ...
      if period.kind == pkInt and period.intVal < 5:
        vr.addWarning(
          wlMedium,
          "Indicator '" & ind.id & "' uses very short period (" &
          $period.intVal & "). " &
