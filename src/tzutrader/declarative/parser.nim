...
proc parseIndicator(node: YamlNode): IndicatorYAML =
  ...
  if node.kind != yMapping:
    raise newException(ParseError, formatError("Indicator must be a mapping",
        result.location))
...
proc parseOperator(s: string, loc: Option[SourceLocation] = none(
    SourceLocation)): ComparisonOp =
...
proc parseSimpleCondition(node: YamlNode): ConditionYAML =
  ...
  if node.kind != yMapping:
    raise newException(ParseError, formatError(
        "Simple condition must be a mapping", loc))
  ...
  if left == "" or op == "" or right == "":
    raise newException(ParseError, formatError(
        "Simple condition must have left, operator, and right", loc))
...
proc parsePositionSizing(node: YamlNode): PositionSizingYAML =
  ...
  of "percent", "percentage":
    ...
    raise newException(ParseError, "Unsupported position sizing type: " &
        sizingType & " (use 'fixed' or 'percent')")
