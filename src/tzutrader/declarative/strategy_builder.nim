...
proc getIntParam(params: Table[string, ParamValue], key: string,
    default: int): int =
...
proc getFloatParam(params: Table[string, ParamValue], key: string,
    default: float): float =
...
proc createIndicator*(indicatorDef: IndicatorYAML): IndicatorInstance =
  ...
  of "stochrsi", "stochastic_rsi":
    ...
    let dPeriod = getIntParam(indicatorDef.params, "dPeriod", 3)
    result = IndicatorInstance(kind: ikSTOCHRSI, stochrsi: newSTOCHRSI(
        rsiPeriod, period, kPeriod, dPeriod))
  ...
  of "ppo", "percentage_price_oscillator":
    ...
    raise newException(BuildError, "Unknown indicator type: " &
        indicatorDef.indicatorType)
...
proc parseReference*(s: DeclarativeStrategy, refStr: string,
    bar: OHLCV): float64 =
...
proc evaluateCondition*(s: DeclarativeStrategy, condition: ConditionYAML,
    bar: OHLCV): bool =
...
method getPositionSizing*(s: DeclarativeStrategy): tuple[
    sizingType: PositionSizingType, value: float] =
