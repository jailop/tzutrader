...
##
## This module provides portfolio tracking, position management, and
...
proc calculatePerformance*(p: Portfolio, 
  ...
  var roundTrips: Table[string, seq[Transaction]] = initTable[string, seq[Transaction]]()
  ...
  if result.totalTrades > 0:
    result.winRate = (result.winningTrades.float64 / result.totalTrades.float64) * 100.0
  ...
  if result.totalTrades > 1:
    ...
    if returns.len > 1:
      ...
      let variance = returns.mapIt(pow(it - avgReturn, 2.0)).sum() / (returns.len - 1).float64
