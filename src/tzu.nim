...
proc loadData(symbol, csvFile, yahoo, coinbase, start, endDate: string): tuple[
    data: seq[OHLCV], symbol: string, kind: DataSourceKind] =
...
proc createStrategy(strategyName: string, params: Table[string,
    string]): Strategy =
...
proc tzu(
  ...
  # Portfolio options
  initialCash = 100000.0, commission = 0.0, minCommission = 0.0,
      riskFreeRate = 0.02,
  verbose = false
): int =
  ...
  if strategy.len > 0:
    ...
    return runStrategyBacktest(strategyObj, symbol, csvFile, yahoo, coinbase, start, endDate,
                               initialCash, commission, minCommission,
                               riskFreeRate, verbose)
